...
class QFunction(nn.Module):
    ...
    def forward(self, rgb_pcd, proprio, pcd, lang_goal_emb, lang_token_embs,
                bounds=None, high_lang_emb=None, high_token_emb=None,target_bounds=None,use_prompt=False,target_voxel_size = 100, action_type =""):
        ...
        # construct target voxel grid

        # target_voxelizer = VoxelGrid(
        #     coord_bounds=target_bounds,
        #     voxel_size=target_voxel_size,
        #     device=self._device,
        #     batch_size=b if self._training else 1,
        #     feature_size=3,
        #     max_num_coords=np.prod([128, 128]) * 4,
        # )
        
        target_voxel_grid = self._voxelizer.coords_to_bounding_voxel_grid(
            pcd_flat, coord_features=flat_imag_features, coord_bounds=target_bounds)

        # swap to channels fist
        target_voxel_grid = target_voxel_grid.permute(0, 4, 1, 2, 3).detach()
...
class QAttentionPerActBCAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        try:
            high_lang_emb = replay_sample['high_lang_emb'].float()
            high_token_embs = replay_sample['high_token_embs'].float()
            target_pose = replay_sample['target_pose'][:,:3]
            # action_type = replay_sample['action_type']
            action_type = ""
        except:
            high_lang_emb = None
            high_token_embs = None
            target_pose = None
            action_type = ""
            pass
        ...
        if target_pose != None:
            target_bounds = torch.cat([target_pose - self._bounds_offset, target_pose + self._bounds_offset], dim=1)
        else:
            target_bounds = None
        device = self._device
        ...
        voxel_grid = self._q(obs,
                            ...
                            lang_token_embs,
                            bounds,
                            high_lang_emb=high_lang_emb, 
                            high_token_emb=high_token_embs,
    ...
    def act(self, step: int, observation: dict,
            deterministic=False,use_prompt=True,target_bounds=None) -> ActResult:
        # deterministic = True
        bounds = self._coordinate_bounds
        # bounds = target_bounds.unsqueeze(dim=0)
        ...
        high_lang_tokens = observation.get('high_lang_tokens', None)
        ...
        with torch.no_grad():
            ...
            if high_lang_tokens is not None:
                high_lang_tokens = high_lang_tokens.long()
                high_lang_emb, high_token_embs = self._clip_rn50.encode_text_with_embeddings(high_lang_tokens)
                high_lang_emb = high_lang_emb.to(self._device)
                high_token_embs = high_token_embs.to(self._device)
            else:
                high_lang_emb = None
                high_token_embs = None
