...
class PerceiverVoxelLangEncoder(nn.Module):
    def __init__(
        ...
        if self.pos_encoding_with_lang:
            ...
            if self.use_prompt:
                self.prompt_pos_encoding = nn.Parameter(torch.randn(1,
                                                            lang_max_seq_len + spatial_size ** 3,
                                                            self.input_dim_before_seq))
        else:
        ...
        # voxel target preprocessing 1x1 conv encoder
        if self.use_prompt:
            self.target_preprocess = Conv3DBlock(
                self.init_dim, self.im_channels*2, kernel_sizes=1, strides=1,
                norm=None, activation=activation,
            )
        ...
        self.patchify = Conv3DBlock(
            ...
            norm=None, activation=activation)
        if self.use_prompt:
            self.prompt_patchify = Conv3DBlock(
                self.target_preprocess.out_channels, self.im_channels*2,
                kernel_sizes=self.voxel_patch_size, strides=self.voxel_patch_stride,
                norm=None, activation=activation)
        ...
        if self.lang_fusion_type == 'concat':
            ...
            if self.use_prompt:
                self.high_lang_preprocess = nn.Linear(lang_feat_dim, self.im_channels)
        elif self.lang_fusion_type == 'seq':
            self.lang_preprocess = nn.Linear(lang_emb_dim, self.im_channels * 2)
            if self.use_prompt:
                self.high_lang_preprocess = nn.Linear(lang_emb_dim, self.im_channels * 2)
        ...
        if not self.use_prompt:
            ...
            self.prompt_latents = nn.Parameter(torch.randn(num_latents, latent_dim))
        ...
        # encoder cross attention
        if self.use_prompt:
            self.prompt_cross_attend_blocks = nn.ModuleList([
                PreNorm(latent_dim, Attention(latent_dim,
                                            self.input_dim_before_seq,
                                            heads=cross_heads,
                                            dim_head=cross_dim_head,
        ...
        if self.use_prompt:
            self.prompts_layers = nn.ModuleList([])
        ...
        if self.use_prompt:
            for i in range(2):
                self.prompts_layers.append(nn.ModuleList([
                    get_latent_attn(**cache_args),
                    get_latent_ff(**cache_args)
                ]))
    ...
    def forward(
        ...
        if prompt == True and high_token_emb is not None and target_voxel_grid is not None:
            # preprocess taget_prompt
            target = self.target_preprocess(target_voxel_grid)    # [B,10,100,100,100] -> [B,128,100,100,100]
            target_ins = self.prompt_patchify(target)                               # [B,128,100,100,100] -> [B,128,20,20,20]
            b, c, d, h, w, device = *target_ins.shape, target_ins.device
            target_ins = rearrange(target_ins, 'b d ... -> b ... d')            # [B,20,20,20,128]
            target_ins = rearrange(target_ins, 'b ... d -> b (...) d')          # [B,8000,128]
            # option 2: add lang token embs as a sequence
            if self.lang_fusion_type == 'seq':
                l = self.high_lang_preprocess(high_token_emb)         # [B,77,1024] -> [B,77,128]
                target_ins = torch.cat((l, target_ins), dim=1)                  # [B,8077,128]

            # add pos encoding to language + flattened grid (the recommended way)
            if self.pos_encoding_with_lang:
                target_ins = target_ins + self.prompt_pos_encoding
            # batchify latents
            latent = repeat(self.prompt_latents, 'n d -> b n d', b=b)

            cross_attn, cross_ff = self.prompt_cross_attend_blocks

            for it in range(self.iterations):
                # encoder cross attention
                latent = cross_attn(latent, context=target_ins, mask=mask) + latent
                latent = cross_ff(latent) + latent

                # self-attention layers
                for self_attn, self_ff in self.prompts_layers:
                    latent = self_attn(latent) + latent
                    latent = self_ff(latent) + latent

        # preprocess input
        d0 = self.input_preprocess(ins)                       # Conv3DBlock:[B,10,100,100,100] -> [B,64,100,100,100]
        
        # aggregated features from 1st softmax and maxpool for MLP decoders
        feats = [self.ss0(d0.contiguous()), self.global_maxp(d0).view(ins.shape[0], -1)]  # SpatialSoftmax3D,nn.AdaptiveMaxPool3d
        ...
        for i in range(self.iterations):
            if prompt == True:
                # encoder cross attention
                latent = cross_attn(latent, context=ins, mask=mask) + latent
                latent = cross_ff(latent) + latent
        ...
        # decoder cross attention
        latents = self.decoder_cross_attn(ins, context=latent)
