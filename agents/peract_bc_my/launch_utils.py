    ...
    for d_idx in range(num_demos):
        ...
        for i in range(len(demo) - 1):
            ...
            if not cfg.method.prompt:
                ...
                high_lang = high_level_description
            action_type = low_lang_list[key_point_index].split(" ")[0]
            # if i == 0:
            #     print(desc)
            # if our starting point is past one of the keypoints, then remove it
            ...
            _add_keypoints_to_replay(
                ...
                clip_model=clip_model, device=device,target_pose=target_pose,high_level_description=high_lang)
    logging.debug('Replay %s filled with demos.' % task)
