...
class Task(object):
    ...
    def decorate_observation(self, observation: Observation) -> Observation:
        ...
        """
        observation.target_objects_information=self.objects_information()
    ...
    def objects_information(self):
        ...
        for name in information: 
            if  "boundary" not in name and "dummy" not in name and "success" not in name and "boundary" not in name and "phys" not in name and "waypoint" not in name:
                # if self.name == "place_shape_in_shape_sorter" or self.name =="put_groceries_in_cupboard":               
                dis = np.abs(np.sum(information[name]["pose"][:2]-information["waypoint1"]["pose"][:2]))
                # print(name,dis)
                if "waypoint" in name:
                    continue
                elif dis < min_dis or name in self.name:
                    min_dis = dis
                    target_object = name
                # for info
        return target_object,information
