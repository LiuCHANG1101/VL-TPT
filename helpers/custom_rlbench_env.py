...
class CustomRLBenchEnv(RLBenchEnv):
    ...
    def reset_to_demo(self, i):
        ...
        # print(self._lang_goal)
         ...
         # self._dataset_root, self._variation_number,self._task.get_name()
        task_root = os.path.join(self._dataset_root, self._task.get_name())
        episode = "episode"+str(i)
        episode_path = os.path.join(
            task_root, "all_variations",
            "episodes" , episode)
        with open(os.path.join(episode_path, 'low_lang.pkl'), 'rb') as f:
            try:
                step_lang = pickle.load(f)
            except EOFError:
                print(episode_path)
                step_lang =  None
        
        # step_lang=tokenize(step_lang).numpy()

        self._previous_obs_dict = self.extract_obs(obs)
        # self._previous_obs_dict["step_lang"] = step_lang
        ...
        return step_lang,self._previous_obs_dict
